    ...
    <script>
        $(document).ready(function() {
            ...
            window.changeRoute = function(route) {
                dateRange(defaultStartDate, defaultEndDate, loadDatas[2].dateRangeId, route, loadDatas[2].content);

                if (typeof(Storage) !== "undefined") {
                    localStorage.setItem('selectedRoute', route);
                } else {
                    alert("Local storage is not supported by your browser. Please use a different browser or enable local storage.");
                }
            ...
            function updateDateRange(startDate, endDate) {
                $('#reportrange2').find('span').html(startDate.format('MMM-DD-YYYY') + ' - ' + endDate.format('MMM-DD-YYYY'));
                // loadDatas.slice(1).forEach(function(data) {
                //     dateRange(startDate, endDate, data.dateRangeId, data.route, data.content);
                // });
